...
public class TrieSearch {
	...
	public static void main(String[] args){
		...
		int phraseCount = 0;
		try {
			Scanner sc = new Scanner(new File("src/articleMatcher/nouns"));
	...
	public static void setMostRelated(Article toSet, List<Article> articles,int number){
		if(!initialised){
			try {
				Scanner sc = new Scanner(new File("src/articleMatcher/nouns"));
				phrases = generateTrie(sc);
		...
		Collections.sort(sortedArticles,new ArticleComparator(toSet));
		//System.out.println("Value: "+toSet.getLink());
		//for(Article a : sortedArticles) System.out.println(a.getLink());
		
		List<String> ans = new ArrayList<String>();
		for(int i=0;i<number;i++) ans.add(sortedArticles.get(i+1).getLink());
		toSet.setRelated(ans);
	}
